...
Now about 1.5 years later, I didn't say what holidays right, The post is finally done!

The work on this has been a constant cycle of:

* 'Why this difference?'
* Ahh... ok!
* Take a few weeks without looking at it...
* 'Where were I? Ahhh soon done... I just have this to check!'
...
> All graphs in this article is generated by my own tool that I don't recommend any one to use, but here is a link anyways! [wccharts](https://github.com/wc-duck/wccharts)
...
> I feel like I'm missing something here... is GCC inlining the copy for x bytes and if the copy is bigger it falls back to memcpy? Need to understand this better.
...
Next up, lets have a look at `-O2/-O3`, here we see that clang still decide to just call `memcpy()` and be done with it while gcc tries to be smart and add an inlined vectorized implementation using the SSE-registers (this is the same vectorization that it uses when its just a pure `memcpy()`).
...
Up until now we have only checked performance on 4MB buffers but what happen in smaller and bigger buffers? Lets add some tests over a range of buffer sizes, we'll add benchmarks on swapping buffers from 16 bytes up to 2GB in "reasonable" intervals and plot them against each other as a byte/sec vs buffer-size. For comparison we'll also add in a pure `memcpy()` perf test as a benchmark as well.

We'll split this section in debug and release build so let the graph-fest begin!

## Different buffer-sizes, `-O0`

Fist of, some graphs for all our implementations.

[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O0_all.png "size vs speed, gcc, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O0_all.png)
[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O0_all.png "size vs speed, clang, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O0_all.png)
...
[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O0_under_12.png "size vs speed, gcc, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O0_under_12.png)
[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O0_under_12.png "size vs speed, clang, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O0_under_12.png)
...
Do we have to mention the "flat-line" that is `std::swap_ranges()`?

Other than this, the both compilers do a similar job an all implementations!

## Different buffer-sizes, `-O2`/ `-O3`

Lets dig into the optimized builds!

[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O2_all.png "size vs speed, gcc, -O2")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O2_all.png)
[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O2_all.png "size vs speed, clang, -O2")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O2_all.png)

First thing we can see here is that over 12MB all implementations just flattens out at "memory speed" except for the generic implementation and `std::swap_ranges()` that on GCC performs really bad while on clang the same as the others.

So lets focus on what happens under the L3-cache boundary and remove the pure-memcpy()!

[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O2_under_12.png "size vs speed, gcc, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_gcc_O2_under_12.png)
[![](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O2_under_12.png "size vs speed, clang, -O0")](/images/swapping-memory-and-compiler-optimizations/memswap_size_vs_speed_clang_O2_under_12.png)
...
* [Improving the State of Debug Performance in C++](https://devblogs.microsoft.com/cppblog/improving-the-state-of-debug-performance-in-c/)
...
* What do MSVC do?
* Is it worth looking into some inline assembly in some parts of the code?
* optimizing (and making sure they work as expected) the other functions in [memcpy_util.h](https://github.com/wc-duck/memcpy_util).

Last words. Was this interesting? Where did I mess up? Want to see more like this? Hit me up on mastodoon and/or "the bird site" and tell me! (As long as you are fairly nice!)
